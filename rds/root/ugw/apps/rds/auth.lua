...
local function macwhitelistset(conn, group, arr)
	rds,pcli = conn.rds, conn.pcli		assert(group and rds and pcli)
	local ret, err = check_maclist(arr)
	if not ret then
		return get_status(1, err)
	end

	return pcli:query_auth({cmd = "macwhitelist_set",data = {group = group, data = arr}}) or get_status(1, "error")

end

local function macwhitelistget(conn, group, data)
	rds, pcli = conn.rds, conn.pcli		assert(group and rds and pcli)
	return pcli:query_auth({cmd = "macwhitelist_get", data = {group = group, data = data}}) or get_status(1, "error")
end

local function macblacklistset(conn, group, arr)
	rds,pcli = conn.rds, conn.pcli		assert(group and rds and pcli)
	local ret, err = check_maclist(arr)
	if not ret then
		return get_status(1, err)
	...
	return pcli:query_auth({cmd = "macblacklist_set",data = {group = group, data = arr}}) or get_status(1, "error")
end

local function macblacklistget(conn, group, data)
	rds, pcli = conn.rds, conn.pcli		assert(group and rds and pcli)
	return pcli:query_auth({cmd = "macblacklist_get", data = {group = group, data = data}}) or get_status(1, "error")
end
...
return {
	...
	macwhitelistset = macwhitelistset,
	macwhitelistget = macwhitelistget,
	macblacklistset = macblacklistset,
	macblacklistget = macblacklistget,
