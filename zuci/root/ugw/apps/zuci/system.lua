...
local function uploadbrush(group, data)
	...
	local upload_img = "/tmp/UploadBrush.img"
	local decrypt_img = "/tmp/decrypt.img"

	local decrypt_cmd = string.format("openssl aes-256-cbc -d -salt -in %s -out %s -k %s", upload_img, decrypt_img, key)
	os.execute("mkdir -p /tmp/UploadBrush")
	os.execute(decrypt_cmd)
	os.execute("tar -zxf /tmp/decrypt.img -C /tmp/UploadBrush/")

	local rdm_str = read(s_random)
	local rdm_map = js.decode(rdm_str)
	if not (rdm_map and rdm_map.bin_random) then
